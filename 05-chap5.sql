...
--vendors - vendorid / AccountNo
--glaccounts - Accountno / accountdescription
--invoicelineitems - accountno / invoiceid
--invoices - invoiceid

--8. which vendors are being paid from more than one account?
SELECT VendorName, COUNT(DISTINCT il.AccountNo) AS NumberOfAccounts
	FROM Vendors v
	JOIN Invoices i
		ON v.VendorID = i.VendorID
	JOIN InvoiceLineItems il
...
SELECT i.VendorID, InvoiceDate, InvoiceTotal,
	...
	JOIN Vendors v
		ON i.VendorID = v.VendorID
	GROUP BY i.VendorID, InvoiceDate, InvoiceTotal;
